...
while menuCycle:
    ...
    greeting = font1.render('Snake', True, (255, 255, 0))
    n_players = font2.render('Choose number of players', True, (0, 204, 0))
    screen.blit(greeting, (300, 150))
    screen.blit(n_players, (200, 320))

    lvlOne = pygame.Surface((180, 50))
    lvlOne.fill((255, 255, 255))
    lvlOne.blit(font3.render('Records Level1', True, (0, 0, 0)), (15, 11))
    screen.blit(lvlOne, (160, 700))

    lvlTwo = pygame.Surface((180, 50))
    lvlTwo.fill((255, 255, 255))
    lvlTwo.blit(font3.render('Records Level2', True, (0, 0, 0)), (15, 11))
    screen.blit(lvlTwo, (460, 700))

    imagelvlOne = pygame.image.load('one_p.png')
    levelOneRect = imagelvlOne.get_rect(bottomright=(330, 600))
    screen.blit(imagelvlOne, levelOneRect)
    imagelvlTwo = pygame.image.load('two_p.png')
    lvlTwoRect = imagelvlTwo.get_rect(bottomright=(600, 600))
    screen.blit(imagelvlTwo, lvlTwoRect)
...
record1 = ''
with open('lvlOne.txt', 'r') as reader:
    xax = (reader.read())
    record1 += xax
s = record1.split('\n')
...
screen.fill((255, 204, 153))
record2 = ''
with open('lvlTwo.txt', 'r') as reader:
    xax2 = (reader.read())
    record2 += xax2
s2 = record2.split('\n')
...
class LevelTwoWalls:
    def __init__(self):
        ...
        b = random.randint(182, 768)
        self.x, self.y = a, b
        self.image = pygame.image.load('cross_icon.png')
...
wall_image = pygame.image.load('cross_icon.png')
    ...
    for i in range(len(snake.elements)):
        if i > 0:
            ...
            if (body_x == head_x and body_y == head_y):
                with open('lvlOne.txt', 'a', encoding='utf-8') as file:
...
while mainCycleLevelTwo:
    ...
    for i in range(len(snake.elements)):
        if i > 0:
            ...
            body_y = snake.elements[i][1]
            if (body_x == head_x and body_y == head_y):
                with open('lvlTwo.txt', 'a', encoding='utf-8') as file:
                    file.write(f'{snake.score}' + '\n')
                game_over()
    if is_in_walls_first():
        with open('lvlTwo.txt', 'a', encoding='utf-8') as file:
